...
from data_mining import covid_county_df, covid_state_df
...
county_cases_fig = px.choropleth(covid_county_df, geojson=counties, locations='fips', 
                    color='cases', color_continuous_scale="orrd",
                    range_color=(0, 200000), color_continuous_midpoint= 25000, 
...
pio.write_html(county_cases_fig, file=f'templates/county_covid_cases.html', auto_open=False)


# Create the map for covid deaths by county
county_deaths_fig = px.choropleth(covid_county_df, geojson=counties, locations='fips', 
...
state_cases_fig = go.Figure(
                data=go.Choropleth(
                locations=covid_state_df['state_abb'], # Spatial coordinates
                z = covid_state_df['cases'].astype(int), # Data to be color-coded
...
state_deaths_fig = go.Figure(
                data=go.Choropleth(
                locations=covid_state_df['state_abb'], # Spatial coordinates
                z = covid_state_df['deaths'].astype(int), # Data to be color-coded
                locationmode = 'USA-states', # set of locations match entries in `locations`
                colorscale = 'reds',
